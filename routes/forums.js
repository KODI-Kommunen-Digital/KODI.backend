...
router.get("/:id/members", authentication, async function (req, res, next) {
    ...
    try {
        var response = await database.get(tables.FORUM_MEMBERS, { forumId: forumId }, null, cityId);
        var memberCityUserIds = await database.get(tables.USER_CITYUSER_MAPPING_TABLE, {
            cityUserId: response.rows.map(x => {
                return x.userId
            })
        }, null)
         var memberUserIds = await database.get(tables.USER_TABLE, {
...
router.post("/:id", authentication, async function (req, res, next) {
    ...
    var memberCityUserId = await database.get(tables.USER_CITYUSER_MAPPING_TABLE, {
...
router.patch("/:id", authentication, async function (req, res, next) {
    ...
    var userDetails = await database.get(tables.FORUM_MEMBERS, { forumId: id, userId: memberCityUserId.rows[0].cityUserId }, null, cityId);
    ...
    response = await database.get(tables.FORUMS, { id:id},null,cityId);
    if (!response.rows || response.rows.length == 0) {
    ...
    database
        .update(tables.FORUMS, updationData, { id:id }, cityId)
        .then((response) => {
            res.status(200).json({
                status: "success",
                data: id
