...
async function sendMessage({ userId, message, sessionId }) {
    ...
    if (!sessionId) {
        ...
        session = await sessionsRepo.create({ data: { userId, name: sessionName } });
        sessionId = session.id;
    } else {
        session = await sessionsRepo.getOne({ filters: [{ key: 'id', sign: '=', value: sessionId }] });
        if (!session) throw new Error('Session not found');
    }
    // Store user message as string (not JSON)
    await chatsRepo.create({ data: { sessionId, sender: 'user', message: JSON.stringify(message) } });
    // Query chatbot
    const botRes = await axios.post(CHATBOT_URL, { query: message });

    // Store bot response as string (not JSON)
    const botMessage = JSON.stringify(botRes.data);
    await chatsRepo.create({ data: { sessionId, sender: 'bot', message: String(botMessage) } });
