...
async function ensureRequesterIsCityAdminForCities(requesterId, cities) {
    ...
    for (const cityId of cities) {
        const isAdmin = await cityUserRolesRepo.isUserCityAdmin(requesterId, cityId);
        if (!isAdmin) unauthorized.push(cityId);
    }
    if (unauthorized.length) {
        throw new AppError(`Not city admin for cities: ${unauthorized.join(",")}`, 403);
    }
}
...
async function upsertModeratorPermissions(moderatorId, permissionIds) {
    ...
    for (const permissionId of permissionIds) {
        await moderatorPermissionsRepository.create({ data: { moderatorId, permissionId } });
    }
}

const createModerators = async function (requesterId, requesterRoleId, payload) {
    try {
        if (!requesterId || !requesterRoleId) throw new AppError("Unauthorized", 401);
        ...
        if (!userId || isNaN(Number(userId))) throw new AppError("Invalid userId", 400);
        if (!Array.isArray(permissions) || permissions.length === 0) throw new AppError("permissions must have at least one item", 400);
        if (!Array.isArray(cities) || cities.length === 0) throw new AppError("cities must have at least one item", 400);
        ...
        if ([roles.Moderator, roles.Admin, roles["City Admin"]].includes(user.roleId)) throw new AppError("User is already a moderator, admin, or city admin", 400);
        ...
        if (created.length > 0) {
            await userRepository.update({ data: { roleId: roles.Moderator }, filters: [{ key: "id", sign: "=", value: userId }] });
        }
...
const updateModerator = async function (requesterId, requesterRoleId, payload) {
    try {
        ...
        const { userId, email, permissions = [], cities = [], removeAbsent = true } = payload || {};
        if (!userId || isNaN(Number(userId))) throw new AppError("Invalid userId", 400);
        if (!Array.isArray(permissions) || permissions.length === 0) throw new AppError("permissions must have at least one item", 400);
        if (!Array.isArray(cities) || cities.length === 0) throw new AppError("cities must have at least one item", 400);
        await ensureRequesterIsCityAdminForCities(requesterId, cities);
        ...
        if (email && email !== user.email) {
            await userRepository.update({ data: { email }, filters: [{ key: "id", sign: "=", value: userId }] });
        }
        ...
        for (const cityId of cities) {
            ...
            if (existing) {
                ...
                const { id: moderatorId } = await moderatorsRepository.create({ data: { cityId, userId, createdBy: requesterId } });
                await upsertModeratorPermissions(moderatorId, permissionIds);
        ...
        if (removeAbsent) {
            ...
            for (const m of toRemove) {
                const isAdmin = await cityUserRolesRepo.isUserCityAdmin(requesterId, m.cityId);
                if (isAdmin) {
                    await moderatorsRepository.delete({ filters: [{ key: "id", sign: "=", value: m.id }] });
                }
            }
        }
...
const deleteModerators = async function (requesterId, requesterRoleId, payload) {
    try {
        if (!requesterId || !requesterRoleId) throw new AppError("Unauthorized", 401);
        if (![roles["City Admin"], roles.Admin].includes(requesterRoleId)) {
            throw new AppError("You are not authorized to perform this action", 403);
        ...
        await userRepository.update({ data: { roleId: roles["Content Creator"] }, filters: [{ key: "id", sign: "=", value: userId }] });
...
const listModeratorsForRequester = async function (requesterId, requesterRoleId) {
    try {
        if (!requesterId || !requesterRoleId) throw new AppError("Unauthorized", 401);
        if (![roles["City Admin"], roles.Admin].includes(requesterRoleId)) {
            throw new AppError("You are not authorized to perform this action", 403);
        ...
        if (requesterRoleId === roles.Admin) {
            ...
            const allMods = await moderatorsRepository.getAll({ columns: "cityId, userId, id" });
            ...
            cityIds = [...new Set((adminCityRows.rows || []).map(r => r.cityId))];
        }
        ...
        for (const uid of userIds) {
            const u = await userRepository.getOne({ columns: "id, username, email, firstname, lastname", filters: [{ key: "id", sign: "=", value: uid }] });
            if (u) users.push(u);
        }
        ...
        // Build permission set per user and collect cityIds for name lookup
        ...
        for (const [uid, arr] of byUser.entries()) {
            ...
            for (const item of arr) {
                ...
                const perms = await moderatorPermissionsRepository.getAll({
                    ...
                    filters: [{ key: "moderatorId", sign: "=", value: item.moderatorId }]
                });
                const permissionIds = (perms.rows || []).map(p => p.permissionId);
                for (const pid of permissionIds) {
                    const p = await permissionsRepository.getOne({ columns: "id, name", filters: [{ key: "id", sign: "=", value: pid }] });
        ...
        // Build response flattened as requested
        return users.map(u => {
            const mods = byUser.get(u.id) || [];
            const cityObjs = [...new Set(mods.map(i => i.cityId))]
                .map(cid => ({ id: cid, name: cityNameMap.get(cid) || null }));
            const permissions = [...(userPermissionsMap.get(u.id) || new Set())];
            return {
                id: u.id,
                username: u.username,
                email: u.email,
...
const getModeratorProfile = async function (requesterId, requesterRoleId, targetUserId) {
    try {
        if (!requesterId || !requesterRoleId) throw new AppError("Unauthorized", 401);
        if (!targetUserId || isNaN(Number(targetUserId))) throw new AppError("Invalid userId", 400);
        const isSelfRequest = Number(requesterId) === Number(targetUserId);
        // Allow self-access; otherwise require Admin or City Admin
        if (!isSelfRequest && ![roles["City Admin"], roles.Admin].includes(requesterRoleId)) {
            throw new AppError("You are not authorized to perform this action", 403);
        }
        ...
        if (!isSelfRequest && requesterRoleId !== roles.Admin) {
            ...
            const adminCityRows = await require("../repository/cityUserRolesRepo").getAll({
                columns: "cityId",
                filters: [
                    { key: "userId", sign: "=", value: requesterId },
                    { key: "isAdmin", sign: "=", value: 1 }
            ...
            const myCities = new Set((adminCityRows.rows || []).map(r => r.cityId));
            const overlap = rows.some(r => myCities.has(r.cityId));
            if (!overlap) throw new AppError("You are not authorized to view this moderator", 403);
        }
        ...
        if (cityIds.size > 0) {
            ...
            for (const c of (cityRows.rows || [])) cityObjs.push({ id: c.id, name: c.name });
        }

        // Basic user info
        const u = await userRepository.getOne({ columns: "id, username, email, firstname, lastname", filters: [{ key: "id", sign: "=", value: targetUserId }] });
