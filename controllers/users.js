...
const login = async function (req, res, next) {
    ...
    try {
        ...
        const loginRes = await userService.login(payload, sourceAddress, head.browsername, head.devicetype);
        res.status(200).json({
            status: "success",
            data: loginRes,
        });
...
const getUserById = async function (req, res, next) {
    ...
    try {
        ...
        const data = await userService.getUserById(userId, cityUser, cityId);
        return res.status(200).json({
            status: "success",
            data,
        });
...
const refreshAuthToken = async function (req, res, next) {
    ...
    try {
        const data = await userService.refreshAuthToken(userId, sourceAddress, refreshToken);
        return res.status(200).json({
            status: "success",
            data
        });
...
const listLoginDevices = async function (req, res, next) {
    ...
    try {
        if (userId !== req.userId) {
            throw new AppError("You are not allowed to access this resource");
        }
...
const getUserListings = async function (req, res, next) {
    try {
        ...
        const data = await userService.getUserListings(userId, pageNo, pageSize, statusId, categoryId, subcategoryId);
        return res.status(200).json({
            status: "success",
            data,
        });
