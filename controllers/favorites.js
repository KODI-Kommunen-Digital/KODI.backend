...
const getAllFavoritesForUser = async function (req, res, next) {
    ...
    try {
        const data = await favoritesService.getAllFavoritesForUser(paramUserId, userId);
        res.status(200).json({
            status: "success",
            data,
        });
...
const getFavoriteListingsForUser = async function (req, res, next) {
    try {
        ...
        const data = await favoritesService.getFavoriteListingsForUser(paramUserId, userId, categoryId, cityId);
        res.status(200).json({
            status: "success",
            data,
        });
...
const addNewFavoriteForUser = async function (req, res, next) {
    try {
        ...
        const id = await favoritesService.addNewFavoriteForUser(paramUserId, userId, cityId, listingId);
        res.status(200).json({
            status: "success",
            id,
        });
...
const deleteFavoriteListingForUser = async function (req, res, next) {
    try {
        ...
        const paramUserId = parseInt(req.paramUserId);
        const userId = parseInt(req.userId);
        await favoritesService.deleteFavoriteListingForUser(favoriteId, paramUserId, userId);
        res.status(200).json({
            status: "success",
        });
    } catch (err) {
