...
router.get("/", async function (req, res, next) {
    try {
        ...
        if (req.query.cityId) {
            if (Number(req.query.cityId)) {
                ...
                if (response.rows && response.rows.length === 0) {
                    return next(
                        new AppError(`Invalid City '${req.query.cityId}' given`, 400),
                    );
                }
        ...
        const responseListings = await database.callQuery(
            queryLisitngs,
            [getDateInFormate(new Date(currentDate - 12 * 60 * 60 * 1000))],
            Number(filter.cityId),
        );
        const dataListings = responseListings.rows;
        if (
            !dataListings ||
      dataListings.length <= 0 ||
      dataListings[0].id !== Number(req.query.listingId)
        ) {
