...
const getCategoryById = async function (categoryId, cityId, isEnabled = false) {
    const response = await database.get(
        tables.CATEGORIES_TABLE,
        { id: categoryId, isEnabled },
        null,
        cityId,
    );
    if (!response || !response.rows || !response.rows.length) {
...
const getCityUserMapping = async function (cityId, userId) {
    const response = await database.get(tables.USER_CITYUSER_MAPPING_TABLE, {
        cityId,
        userId,
    });
...
const createListingImage = async function (
    cityId,
    listingId,
    imageOrder,
    logo,
...
const createListingImageWithTransaction = async function (
    listingId,
    imageOrder,
    logo,
    transaction,
...
const deleteListingImageById = async function (id, cityId) {
    await database.deleteData(tables.LISTINGS_IMAGES_TABLE, { id }, cityId);
};

const deleteMultipleListingImagesById = async function (ids, cityId) {
    await database.deleteData(tables.LISTINGS_IMAGES_TABLE, { id: ids }, cityId);
};
...
module.exports = {
    ...
    deleteMultipleListingImagesById,
};
